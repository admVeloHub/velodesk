/**
 * CgTicketList — lista de demandas RA (layout CRM)
 */
import React from 'react';
import { CG_GROUPS, getStatusLabel, formatSlaRestante } from '../../../services/especiais/consumidorGovData';
import {
  formatRaListDate,
  getRaSlaClass,
  getRaSlaLabel,
} from './cgTicketFormatters';

export default function CgTicketList({
  activeGroup,
  activeCgId,
  activeSort,
  items,
  ...
  const groupName = CG_GROUPS.find((g) => g.id === activeGroup)?.label || '';
      ...
      <div className="ra-crm-list__inner">
        ...
        <ul className="ra-crm-ticket-cards">
          {items.length === 0 ? (
            ...
            items.map((item) => {
              const isActive = String(item.id) === String(activeCgId);
              ...
              return (
                ...
                >
                  <div className="ra-crm-ticket-card__top">
                    ...
                    <span className={`ra-badge ra-badge--${item.statusGov}`}>
                      {getStatusLabel(item.statusGov)}
                    </span>
                  </div>
