...
export default function WorkflowPanel() {
  ...
  const teamQueueId = useMemo(
    ...
    [hasWorkflowAccess, permsCtx?.permissions],
  );
  ...
  const view = useMemo(
    () => (hasWorkflowAccess ? computeWorkflow360ViewForUser(permsCtx?.permissions) : { kpis: [], sections: [] }),
    [hasWorkflowAccess, permsCtx?.permissions],
  );
      ...
      <div className="ws-workflow-desk ws-agent-desk--operational" id="wsWorkflowDesk">
        <div className="ws-hero ws-hero--workflow">
          <div>
            ...
            <h3>Visão Workflow indisponível</h3>
            <p>Esta visão exige a permissão <strong>Visão Workflow</strong> nos overrides da função do agente.</p>
          </div>
        </div>
      </div>
    );
