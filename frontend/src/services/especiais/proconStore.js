...
function matchesSearch(item, query) {
  ...
  return (
    ...
    || String(item.orgaoProcon || '').toLowerCase().includes(q)
...
function matchesChip(item, chipId) {
  ...
  switch (chipId) {
    case 'nao-respondidas':
      return item.statusPc === PC_STATUS.NAO_RESPONDIDA || item.groupKey === 'nao-respondidas';
    case 'abertas':
      return item.aberta;
    case 'workflow-ativo':
...
export function getProconKpis(items = loadAllDemandas()) {
  ...
  const naoRespondidas = operational.filter((i) =>
    i.statusPc === PC_STATUS.NAO_RESPONDIDA || i.groupKey === 'nao-respondidas',
  ).length;
  const respondidas = operational.filter((i) =>
    i.statusPc === PC_STATUS.RESPONDIDA || i.statusPc === PC_STATUS.AGUARDANDO_AUDIENCIA,
  ).length;
  ...
  return [
    ...
    { id: 'orgaos', label: 'Órgãos distintos', value: String(new Set(operational.map((i) => i.orgaoProcon).filter(Boolean)).size), tone: 'success', icon: 'ti-building-community' },
  ];
}

export function groupDemandasByStatus(items) {
  return PC_GROUPS.map((group) => ({
    ...group,
...
export function getKanbanColumns(items) {
  ...
  const cols = [...PC_GROUPS.filter((g) => g.id !== 'respondidas'), extra, PC_GROUPS.find((g) => g.id === 'respondidas')];
  return cols.map((col) => {
    let colItems = [];
...
export function getReportSeries(items = loadAllDemandas()) {
  const operational = items.filter((i) => !isEspeciaisItemFinalizada(i));
  const byStatus = {
    'Não respondida': operational.filter((i) => i.statusPc === PC_STATUS.NAO_RESPONDIDA).length,
    'Workflow ativo': operational.filter((i) => i.workflowAtivo).length,
    Respondida: operational.filter((i) => i.statusPc === PC_STATUS.RESPONDIDA).length,
    'Aguard. audiência': operational.filter((i) => i.statusPc === PC_STATUS.AGUARDANDO_AUDIENCIA).length,
  };
  const slaBuckets = {
  ...
  const byOrgao = operational.reduce((acc, item) => {
    const orgao = item.orgaoProcon || 'Não informado';
...
export function getFooterSummary(items, selectedCount = 0) {
  const naoResp = items.filter((i) => i.statusPc === PC_STATUS.NAO_RESPONDIDA).length;
...
export function buildRegistroDefaults(item = {}) {
  ...
  return {
    protocoloProcon: item.protocoloProcon || generateProtocolo(),
    ...
    whatsappMensagem: item.whatsappMensagem || PC_WHATSAPP_DEFAULT_MSG,
...
export function createEmptyDemanda() {
  ...
  return {
    ...buildRegistroDefaults({
      protocoloProcon: generateProtocolo(),
      isDraft: true,
    }),
    id,
  };
