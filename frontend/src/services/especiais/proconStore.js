...
export function getProconKpis(items = loadAllDemandas()) {
  ...
  return [
    ...
    { id: 'orgaos', label: 'Órgãos distintos', value: String(new Set(items.map((i) => i.orgaoProcon).filter(Boolean)).size), tone: 'success', icon: 'ti-building-community' },
  ];
}
...
export function getReportSeries(items = loadAllDemandas()) {
  const byStatus = {
    'Não respondida': items.filter((i) => i.statusPc === PC_STATUS.NAO_RESPONDIDA).length,
    'Workflow ativo': items.filter((i) => i.workflowAtivo).length,
    Respondida: items.filter((i) => i.statusPc === PC_STATUS.RESPONDIDA).length,
    'Aguard. audiência': items.filter((i) => i.statusPc === PC_STATUS.AGUARDANDO_AUDIENCIA).length,
  };
  const slaBuckets = {
